...
data(dim032)
data("kddcup04bio")
toproc <- list(dim1024, dim512, dim256, dim128, dim064, dim032)
vises <- list()
clusters <- list()
for (i in 1:length(toproc)) {
	dat <- t(scale(as.matrix(toproc[[i]])))
...
```{r highd2,eval=F}
load(system.file("extdata/vises2.Rda", package = "clusteringdatasets"))
load(system.file("extdata/clusters2.Rda", package = "clusteringdatasets"))
names <- c("dim1024", "dim512", "dim256", "dim128", "dim064", "dim032")
par(mfrow = c(2, 3), mar = c(0,0,1,0))
for (i in 1:length(names)) {
	df <- data.frame(t(vises[[i]]$coords))
	colnames(df) <- c("x", "y")
